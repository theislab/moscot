...
class LCA_distance(TreeCostFn):
    def compute_distance(self, tree) -> jnp.ndarray:  # TODO: specify Any, i.e. which data type trees have
        ...
        for nodes, an in all_pairs_lowest_common_ancestor(tree):  # TODO: rewrite to make it more efficient
            cost[int(nodes[0]), int(nodes[1])] = nx.dijkstra_path_length(tree, an, nodes[0]) + nx.dijkstra_path_length(
                tree, an, nodes[1]
            )
...
def _cell_cost_from_matrix(
    adata: AnnData, key: str, key_value: Union[str, int], pre_cost: jnp.ndarray, distance_key: str = "core"
) -> jnp.ndarray:  # TODO: define key_value dtype better
    ...
    for i, ci in enumerate(adata_filtered.obs_names):
        C[i, :] = pre_cost[
            adata_filtered.obs.loc[ci, distance_key].values, adata_filtered.loc[adata_filtered.obs_names, distance_key]
        ]
    return C

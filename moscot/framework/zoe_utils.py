...
def fit_lineage_coupling(adata, time_1, time_2, lineage_tree_t2,
    ...
    # compute cost matrix
    lineageOT_cost = ot.utils.dist(state_arrays['early'], ancestor_info[0]) @ np.diag(
        ancestor_info[1] ** (-1))
...
def create_adatas(adata, dep):
    ...
    adatas['top'] = adatas['orig'][adatas['orig'].obs['cellID'].isin(top_cell_ids)][
        [str(c) for c in top_cell_ids]].copy()
...
def compute_dists(adatas, dep, true_tree=True, state_key='X_pca_50'):
    ...
    d = add_node_depths(adatas['processed'], G)
    ...
    joint = create_geometry(
        ot.utils.dist(adatas['joint'][adatas['joint'].obs['time_point'] == 0].obsm[state_key],
...
def true_coupling(adatas, dep, state_key='X_pca_50', normalize=True):
    ...
    ancestor_info = np.zeros((len(bottom_leaves), adatas['bottom'].obsm[state_key].shape[1]),
                             dtype=float)
    ...
    for i, (top_leaf, cs) in enumerate(children.items()):
        ...
        for c in cs:
            ...
            data = np.asarray(
                tmp_adata[(tmp_adata.obs['cellID'] == top_leaf + 1)].obsm[state_key]).squeeze()

...
from moscot.mixins._base_analysis import CompoundAnalysisMixin
...
class TemporalAnalysisMixin(CompoundAnalysisMixin):
    ...
    def _interpolate_gex_with_ot(
        ...
        if adjust_by_growth:
            transport_matrix = solver.transport_matrix / np.power(
                solver.transport_matrix.sum(axis=0), 1.0 - interpolation_parameter
            )
        else:
        ...
        return np.asarray(
            [
                adata_1.X[i // adata_2.n_obs] * (1 - interpolation_parameter)
                + adata_2.X[i % adata_2.n_obs] * interpolation_parameter
                for i in choices
            ],
            dtype=np.float64,
        )

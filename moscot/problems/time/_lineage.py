...
class TemporalBaseProblem(MultiMarginalProblem):
    ...
    def _estimate_marginals(
        self, adata: AnnData, source: bool, proliferation_key: Optional[str] = None, apoptosis_key: Optional[str] = None, **kwargs: Any
    ) -> npt.ArrayLike:
...
class TemporalProblem(TemporalAnalysisMixin, SingleCompoundProblem):
    ...
    def _create_problems(self, init_kwargs: Dict[Any, Any] = {}, **kwargs: Any) -> Dict[Tuple[Any, Any], TemporalBaseProblem]:
        return {
    ...
    def _validate_args(self, arg: Union[str, Mapping[str, Sequence[Any]]]) -> Tuple[Union[str, Sequence], Sequence]:
        ...
        print(self.adata.obs[_key].dtype)
        ...
        **kwargs: Any,
        ...
        if forward:
            ...
            for subset in _early_cells:
                try:
                    result = self.push(start=start, end=end, data=_early_cells_key, subset=subset, normalize=True, return_all=False, **kwargs)
                except ValueError as e:
                    if "test123" in str(e): #TODO: adapt
                        logging.info(
        ...
        for subset in _late_cells:
            ...
            try:
                result = self.pull(start=start, end=end, data=_late_cells_key, subset=subset, normalize=True, return_all=False, **kwargs)
            except ValueError as e:
                if "test123" in str(e): #TODO: adapt
                    logging.info(
                        f"No data points corresponding to {subset} found in `adata.obs[groups_key]` for {start if forward else end}"
            ...
            target_cell_dist = (
                df_early[df_early[_early_cells_key].isin(_early_cells)].groupby(_early_cells_key).sum()
            )
            transition_table.loc[:, subset] = [
                target_cell_dist.loc[cell_type, "distribution"]
                if cell_type in target_cell_dist.distribution.index

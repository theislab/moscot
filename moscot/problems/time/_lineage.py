    ...
    def cell_transition(
        ...
        **kwargs: Any,
        ...
        if isinstance(late_cells, str):
            ...
            assert set(_late_cells).isin(
                set(self.adata.obs[_late_cells_key].unique())
            ), f"Not all values {_late_cells} could be found in column {_late_cells_key}"
        if isinstance(early_cells, str):
            ...
            assert set(_early_cells).isin(
                set(self.adata.obs[_early_cells_key].unique())
            ), f"Not all values {_early_cells} could be found in column {_early_cells_key}"

...
from moscot.utils import _get_marginal, _get_differences, _verify_marginals
...
class TemporalProblem(TemporalAnalysisMixin, CompoundProblem):
    ...
    def prepare(
        ...
        ] = None,  # TODO: check differnece with mappringproxytype
        ...
        **kwargs: Any,
        ...
        if self._n_growth_rates_estimates is not None:
            if policy != "sequential":
                raise ValueError(
                    "Growth estimates are only available for sequential transport maps. TODO: CHANGE"
                )  # TODO: @MUCDK change
        ...
        if self._n_growth_rates_estimates is None:
            ...
            for i in range(self._n_growth_rates_estimates):
                ...
                for subset, problem in self._problems.items():
                    ...
                    if scale_by_time:
                        self._a_marg_container.loc[mask, current_key] = np.power(
                            growth_array, 1.0 / self._time_deltas[subset]
                        )
                    else:
    ...
    def pull_back(
        ...
        if (start, end) not in self._problems.keys():
            raise ValueError(f"No transport map computed for {(start, end)}. Try calling 'push_back_composed' instead.")
        if mass is None:
            return self._apply(
                data=key_groups,
                subset=groups,
            ...
            return self._apply(
                data=mass, start=start, end=end, normalize=normalize, return_all=return_all, forward=False
            )
    ...
    def compute_backward_fate(
        ...
        """
        if (start, end) not in self._problems.keys():
            logging.info(f"No transport map computed for {(start, end)}. Trying to compose transports.")
            pairs = self._policy.chain(start, end)
        else:
            pairs = [(start, end)]
        ...
        if target_cell_populations is None:
            ...
            assert set(target_cell_populations) <= set(
                target_adata.obs[key_groups].values
            ), f"Not all cells given are found at time {pairs[-1][1]}"
    ...
    def cell_type_transition(
        ...
        if start_cell_types is None:
            ...
            assert set(start_cell_types) <= set(
                source_adata.obs[key_groups].values
            ), f"Not all cells given are found at time {pairs[-1][1]}"
    ...
    def validate_by_interpolation(
        self, start: Any, end: Any, intermediate: Optional[Any] = None, interpolation_parameter: int = 0.5
    ):
        """
        ...
        if (start, end) not in self._problems.keys():
            raise ValueError(f"No transport map computed for {(start, end)}. Trying to compose transports.")
        ...
        gex_ot_interpolated = self._interpolate_gex_with_ot(
            intermediate_adata.n_obs, source_adata, target_adata, self._problems[(start, end)]._solver
        )
        distance_gex_ot_interpolated = self._compute_wasserstein_distance(
            intermediate_adata, AnnData(gex_ot_interpolated)
        )
        ...
        distance_gex_randomly_interpolated = self._compute_wasserstein_distance(
            intermediate_adata, AnnData(gex_randomly_interpolated)
        )
        if self._last_growth_key is not None:
            ...
            gex_randomly_interpolated_growth = self._interpolate_gex_randomly(
                intermediate_adata.n_obs,
                source_adata,
                target_adata,
                self._problems[(start, end)]._solver,
                self._a_marg_container.iloc[intermediate_indices, -1],
            )
            distance_gex_randomly_interpolated_growth = self._compute_wasserstein_distance(
                (intermediate_adata, AnnData(gex_randomly_interpolated_growth))
            )
        else:

...
class TestGENOTLinProblem:
    ...
    def test_prepare(self, adata_time: ad.AnnData):
        ...
        problem = problem.prepare(key="time", joint_attr="X_pca", condition_attr={"attr": "obs", "key": "time"})
        ...
        assert isinstance(container, NeuralDistribution)
        assert isinstance(container.shared_space, jnp.ndarray)
        assert isinstance(container.condition, jnp.ndarray)
        assert container.shared_space.shape == (n_obs_0, 50)
        assert container.incomparable_space is None
        assert container.condition.shape == (n_obs_0, 1)
    ...
    def test_reproducibility(self, adata_time: ad.AnnData):
        ...
        problem_one = problem_one.prepare(
            key="time", joint_attr="X_pca", condition_attr={"attr": "obs", "key": "time"}, seed=0
        )
